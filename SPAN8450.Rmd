...
Si hacemos este primer mandato, se produce una mensaje que nos informa que se encontraron 20160 casos de la palabra "love" en el BNC.^[Se puede explorar un objeto de 'cooccurences' mediante el htmlwidget como `as(love_co,"htmlwidget")`] El segundo mandato (`as.data.frame`) produce un data frame de la lista de co-ocurrencias. 

```{}
head(love_df)
```
Este data frame muestra que un colocado prominente de la palabra "love" es "affair". Las filas están en orden por su estadística de la prueba del chi-cuadrado, y el colocado "affair" aparece en la primera fila. Si consideramos algunos de las otras columnas, observamos que la selección de casos donde "love" occur con "affair" son 3079 casos. De todos estos casos, 182 de ellos se observaron dentro de la ventana de las cinco palabras que siguen la palabra "love". 2897 de estos casos no se observaron en esta ventana. Podemos utilizar estos números para crear una tabla de contingencia.

### La tabla de contingencia

Los números en el data frame `love_df` son suficientes para completar las celdas grises de la tabla 3.1 de Brezina (página 70). Se presentan en la siguiente tabla.

**Tabla 1**: Tabla de contigencia para una ventana de colocaciones de cinco palabras a la derecha del nodo

| colocado | colocado presente ('affair') |	colocado ausente	| Totales  |
|:-----|:-----|:-----|:-----|
| cerca de "love"	| 182	| 100618 | 20160 casos × 5 ventana = `love_co@size_coi` |
| no cerca de "love"	| 2897 |	112032504 | `love_co@size_ref =112035401` |
| Totales	| 3079	| 12133122 | `size("BNC")` - 20160 = 112136201 |

Sin consideramos la ventana de cinco palabras a la derecha, tenemos aproximadamente 20150*5=100800 oportunidades para la co-ocurrencia.^[Esta estimación es aproximado porque la atestación final puede estar tan cerca al principio o al final del corpus que la ventana de colocación se corta. De manera similar, el investigador puede imponer unas restricciones que determinen que la ventana no pueda extender más allás de las fronteras de una oración. Estas restricciones requieren que las fronteras oracionales sean anotadas en el corpus--por ejepmlo, con los atributos estructurales como <s> y </s>.] Podemos pensar de cada caso o (a) como parte del conjunto de casos con constan del `love_co@size_coi` o (b) como parte del corpus de referencia, es decir, los casos en el BNC que no se ubican cerca de cualquier instancia de la palabra "love". La cantidad de estos casos equivale a `love_co@size_ref`.^[El signo `@` extrae un valor de un luegar de un objeto definido dentro del [S4 object system](http://adv-r.had.co.nz/OO-essentials.html#s4). Es paralelo a la notación de `4` que describió Jonathan Jones en su introducción a R. 
it's analogous to the dollar notation that you learned about in Jonathan Jones' introduction to base R.]


### La frecuencia experada del colocado

Ya podemos calcular lo que llama Brezina el **random co-ocurrence baseline**. Esto se refiere al número esperado de veces que se debería observar "affair" como co-ocurrencia dentro de la ventana de cinco palabras de "love" si las frecuencias fueran el producto sus probabilidades de atestación individual.
...
El denominador es el total completo: el tamaño de corpus menos la cantidad de los casos del nodo. Por definición, estos no so candidatos para la co-ocurrencia (leer "The most sensible definition...", Evert, página 27). El numerador es el producto de la frecuencia del nodo, la frecuencia del colocado, y el tamaño de la ventana presentados mediante la expresión 3.2 en la página 69 de Brezina. El valor numérico de esta fracción es como se presenta en la primera columna del data frame `love_df`--es decir, en la columna `exp_coi`. Esta columna es la frecuencia esperada de la palabra "affair" como un colocado a la derecha de "love". Por supuesto, este número es solamente el valor de la celda E_11 de la matriz de frecuencias esperadas.

**expected frequency of collocation (corrected)**
= $\frac{(node frequency)\times(collocate frequency)\times(window size)}{no. of tokens in text or corpus}$

### Las frecuencias observadas y esperadas

Acordáos que la prueba del chi cuadrado utiliza la descrepancia entre la matriz de frecuencias esperadas y la matriz de frecuencias observadas. Completamos la segunda matriz a base de los resultados sacados de `cooccurences` con tal de que distinguamos cuidadosamente entre los casos que se ubican cerca de "love" y los demás.
...
Calculamos los números marginales para llegar a las filas y las columnas que vimos en la Tabla 1.
...
Con estos números, podemos calculamos los números de las atestaciones esperados según la hipótesis nula.
...
Dado el tamaño tan grande del corpus, tenemos que realizar los cálculos con los logaritmos, donde la multiplicación se convierte en la suma.
...
Recuperamos el valor del `chisquare` que ya calculamos con la función `cooccurences` que se encontró en la primea fila del data frame `love_df`. Esta demostración nos destaca el hecho de que el rango de los colocados a la derecha de "love" es la misma estadística del chi-cuadrado que ya hemos utilizado para probar las hipótesis. ¿Habrá otras maneras de producir un rango de los colocados?

## Las medidas de asociación

Ya hemos visto el uso de las pruebas del chi-cuadrado para los colocados de la palabra "love".
...
En este caso, la palabra "affair" no resulta ser el colocado con el rango más alto. Los resultados de esta segunda métrica la tiene en la novena posición.

El método por defecto `method=ll` produce una tabla de contingencia con probabilidades de 'log-likelihood'.^[Un trabajo yaclásico que defende la idea de utilizar los log-likelihood es el trabajo de [Dunning (1993)](https://www.aclweb.org/anthology/J93-1003/).] Esta métrica representa una medida de asociación diferente que pone los pronombres "you", "him", y "her" en posiciones más altas en en rango que la de "affair". Sin examinamos la columna de `count_partition`, observamos que estas palabas tienen frecuencias de observación mucho más altas que la palabra "affair". Evidentemente, la métrica log-likelihood prioriza *la frecuencia** sobre **la exclusividad**. Hay más o menos cuarenta medidas de asociación descritas por [Evert](http://www.collocations.de/AM/) y que se han implementado en [el UCS/R System](http://www.collocations.de/software.html). Este programa se puede utilizar con nuestro servidor.


### Describrir patrones

Consideremos las palabras "bachelor" y "spinster" en el BNC. Esta palabras se refieren a los hombres y las mujeres solteros, respectivamente. ¿Podemos utilizar el corpus para entender los estereotipos asociados con estas palabras? Vamos a ver los colocados:
...
Parece que la palabra "eligible" es un colocado exclusivo de "bachelor". Sin embargo, las palabras "a", "was" y "his" no parecen ser relevantes. Es común excluir palabras como esas (los "stopwords") de la concordancia, por ejemplo mediante el uso de la función `subset` de PolmineR.
...
Con estas concordancias, se nota el uso del operador de R `%in% que se utiliza para comparar una palabra con una lista pre-determinada de palabras (es decir, el "stoplist") que no importan. El signo de interrogación `!` es el operador negación de R que permite definir una condición que especifica las palabras que **no** son relevantes.

Podemos utilizar el paquete `ggplot2` para examinar los resultados con un "Cleveland dotchart". El objecto que produce la función `cooccurrences` incluye una opción que llama `stat`, lo cual representa [un tipo de data frame especial](https://rdatatable.gitlab.io/data.table/). La expresión `1:14` crea [un vector de números](https://cran.r-project.org/doc/manuals/r-release/R-intro.html#Generating-regular-sequences) con un orden ascendente; este vector de índice ([caso 2 en los R docs](https://cran.r-project.org/doc/manuals/r-release/R-intro.html#Index-vectors)) dentro de los corchetes limita la gráfica a los 14 colocados más prominentes.
...
![](images/BachelorPlot.png)
![](images/SpinsterPlot.png)
Se observa que, con la palabra "spinster", la palabra "elderly" (y también "dried-up", "cold-hearted", "sex-starved" y "rape") tiene un rango alto, lo cual parece apoyar la idea de que es un término despectivo en en inglés británico. En el capítulo 5 de Baker (2006, *Using corpora in discourse analysis*), se encuentra mucha más información sobre este tema.

## La frecuencia de ocurrencia

La medida de asociación más simple es la frecuencia cruda de los colocados posibles. Esta medida se indica como "ID #1" en la tabla 3.3 de Brezina (página 72). Ya hemos visto que la palabra "love" coloca con la palabra "affair" con una frecuencia de 182 en el BNC. ¿Existen pares de palabras que tienen frecuencias más altas de atestación? Este cálculo se realiza mediante la función `ngrams` de PolmineR.
...
Y, naturalmente, podemos extender este proceso a los grupos de tres.
...
La siguiente tabla muestra que el "trigram" más atestado en el BNC es "I don't", seguido por "one of the".

| palabra_1 | palabra_2 |	palabra_3	| Total  |
|:-----|:-----|:-----|:-----|
| 1. I | do | n't | 37090 |
| 2. one | of | the | 29737 |
| 3. the | end | of | 20699 |
...
Esta extensión hacia grupos más grandes también se puede hacer con otras medidadas de asociación mediante la expansión de la tabla de contingencia. Un ejemplo de este tipo de implementación es [el ngram statistics package de Ted Pedersen](https://metacpan.org/pod/release/TPEDERSE/Text-NSP-1.31/lib/Text/NSP/Measures/3D/MI.pm), lo cual produce medidas de 2, 3, o 4 dimensions.

## Colocados de la derecha de un nodo complejo

Podemos replicar el trabajo de Scott y Tribble (2006) con una consideración del grupo "one of the" como un nodo. ¿Cómo varían sus colocados de la derecha como función del género ('genre')?
...
Ya que hemos producido una lista de los colocados a la derecha incluídos en `Academic Text` y `Spoken Conversation`, podemos determinar cuales son los más atestados. Si tratamos los vectores como conjuntos, podemos descalificar las palabras como "first" que coloca frecuencamente con esta expresión ("one of the") en textos de los dos géneros.
...
Estos resultados confirman el argumento de Scott y Tribble que la expresión "one of the main" y "one of the major" son expresiones distinctamente académicas.
...
Descansemos unos minutos de nuestras preguntas lingüísticas para hablar de los autos. Cuando pensamos en los autos, cuanto más grande sea el motor, peor será el millaje. Esta observación se puede verificar con [los datos mpg](https://ggplot2.tidyverse.org/reference/mpg.html) incluidos con `ggplot2`.
...
Como se nota que, para cada litro adicional de desplazamiento (es decir, el espacio dentro de los cilindros de un motor), la economía del uso de gasolina (en la carretera) se empeorará por un factor de tres medio miles por galón (mpg). Este proceso empieza a los máximos teóricos de casi 35 o 36 mpg.

Podemos representar esta relación mediante la siguiente ecuación:

-  $y = -3.5x + 35.7$

Aquí, la variable *y* se refiere a la economía del uso de gasolina y *x* es el tamaño del motor en litros. Podemos crear ('fit') un modelo con la función `lm` en R.

```{r echo=TRUE}
m <- lm(hwy~displ,data=mpg)
```

El objetivo de la función de `lm` es encontrar el mejor multiplicador para $x$ (el "slope") y el mejor constante (el "intercept") para los datos. El primero argumento para `lm` se conoce como una fórmula modelo; la tilde se entiende como "se entiende como función de"--es decir, la economía de la gasolina se modela como función del desplazamiento del motor.

La función auxiliar `coefficients` extrae los valores de un "fitted model" como el modelo `m`.

```{r echo=TRUE}
coefficients(m)
```

El modelo que resulta es "linear" en el sentido de que los resultados se encuentran a lo largo de una línea recta. Podemos predecir la economía para el 2020 Chevy Tahoe, lo cual no fue parte de nuestro conjunto original de datos. Este auto tiene un motor de 5.3 litros.
...
Predecimos que el Tahoe debe obtener un mpg de 16. De hecho, el EPA indica que este auto tiene un mpg de 20 en la carretera. Este rango del EPA es mejor que habríamos esperado basado solo en el desplazamiento. Evidentamente, la tecnología ha avanzado desde el añol 2008. Puede ser que Chevrolet haya mejorado otros aspectos del Tahoe, por ejemplo su aerodinámico. Si tuviéramos los datos sobre, por ejemplo, el área frontal de estos vehículos, podríamos añadirlos como otro indicador para el model--`hwy ~ displ + fontal`. Esto sería un caso de regresión *múltiple*. Con la regresión múltiple, los coeficientes se conocen como los "betas" y se identifican con los subíndices--$\beta_1$ es el coeficiente del primer indicador $x_1$, $\beta_2$ es el coeficiente del segundo indicador $x_2$, etc.

## Elegir entre dos realizaciones alternativas

En la lingüística, el resultado de interés es, para muchos estudios, categórico. Un ejemplo de Brezina (capítulo 4) es la definitud: si se utiliza el artículo definido "the" o el artículo indefinido "a". Otro ejemplo es la alternancia dativa donde el mismo significado semántico se puede comunicar con una estructura dativa preposicional o una estructura de doble complemento. Estas estructuras alternativas se pueden clasificar de la siguiente manera:

| nombre | ejemplo |	subcat	|
|:-----|:-----|:-----|
| estructura dativa preposicional | ...gave toys to the children | V SN(NP) SP(PP) |
| estructura de doble complement | ...gave the children the toys | V SN(NP) SN(NP) |

**Tabla 1:** Los términos que se usan con la alternancia dativa ( (Bresnan et al. 2007, página 70)

![Ejemplo de la estructura dativa preposicional del corpus Switchboard (disponible en nuestro servidor)](images/DativeAlternation.png)
Todavía podemos predecir las atestaciones de categorías lingüísticas como estos con un modelo linear. Sin embargo, tenemos que generalizar el modelo para considerar el hecho de que solo haya dos opciones (preposicional vs. doble objeto). En vez de proponer un modelo de la economía de la gasolina en términos de mpg, podemos modelar la probabilidad de que la estructura dativa preposicional sea elegida o no. Una complicación que nos enfrentará es que las probabilidades tienen que encontrarse en el intervalo $[0,1]$. Podemos utilizar la función logística para imponer un valor artibrario $arb$ en el intervalo.^[El símbolo $e$ en la definición de la función logística denota [el número de Euler](https://mathworld.wolfram.com/e.html).] La figura 1 ilutra este efecto:

$$\frac{e^{arb}}{e^{arb} + 1}=\frac{1}{1+e^{-arb}}$$

![Función logística (de la figure 12.4 de página 202 de Winter 2020)](images/LogisticFunctionWinter.png)
Entonces, para conseguir las probabilidades, tenemos que "squash" algo con la función logística. ¿A qué se refiere el "algo"? Es el "log-odds". Es decir, es el logoritmo de las probabilidades de un evento.

-  **función de logit (aka log-odds)**

$$logit(P(event)) = ln(\frac{P(event)}{1-P(event)})$$

Cuanto mejor los log-odds, más alto será la probabilidad. Estas dos funciones son inversas de una y otra (vé la figura 12.5 de Winter 2020). Si utilizamos un indicador linear para modelar estos log odds, creamos un enlace entre este indicador linear a través de la función logit. Producimos una probaibilidad por el proceso de introducir estos log odds a la función logística como el $arb$. Esto es los que describe Brezina en la página 126 cuando comenta que la regresión logística opera de manera "interna" con los log odds.

Bresnan et al. (2007) estudiaron la alternancia dativa con la colección de Switchboard (3 millones de palabras) que contiene conversaciones telefónicas. Bresnan y sus colegas querían predecir la realización del recipiente como un SN o SP. Notáos que el recipiente es una noción semántica mientras que los elementos SN/NP y SP/PP son conceptos sintácticos. Estos investigadores querían determinar los factores que influían en esta decisión sintáctica. Consideraron una variedad de diferentes variables independientes, como la animicidad (si el reference de la expresión es un ser vivo), la duración de la palabras y la pronominalidad del tema y el recipiente (el tema es otra noción semántica; es decir, "la cosa que se transfiere en mediante la acción denotada por el verbo").^[Esta characterización del trabajo de Bresnan et al. (2007) se ha tomado directamente de la sección 1.2 del libro de R. Harald Baayen, [Analyzing linguistic data](http://www.sfs.uni-tuebingen.de/~hbaayen/publications/baayenCUPstats.pdf).]

Los datos de Bresnan son disponibles con el paquete de R `languageR`.
...
head(dative)
```
R tiene documentación para unos "generalized linear models". Solo tenemos que elegir la regresión logística mediante el ajuste "binomial". Esto signfica que las respuestas ocurren o no de manera categórica. Por defecto, este proceso establece la función enclace con el valor del logit. Hagamos ahora un model simple que predice el tipo de sintagma sintáctica del recipiente basado solo en el estatus de animicidad del recipiente.

```{r echo=TRUE}
dative_mdl <- glm(RealizationOfRecipient ~ AnimacyOfRec, data=dative, family="binomial")
```

Después, pedimos que R nos dé un resumen. Esto nos produce una tabla con los p-values de una prueba contra el hipótesis nula que el indicador (animicidad) no tiene un efecto:

```{r echo=TRUE}
summary(dative_mdl)
```
Considermos la tabla debajo de "Coeficientes". El signo negativo del Intercept significa que, en general, la probabilidades van en contra de las estructuras preposicionales. Este resultado tiene cierto sentido dado que representan el grupo minoritario.

```{r echo=TRUE}
table(dative$RealizationOfRecipient)
```

El signo positivo de la estimación en la fila titulada "AnimacyOfRecinanimate" significa que el log odds de obtener una estructura dativa preposicinal se mejoran un poquito si el recipiente es "Inanimate". Notemos que ser inanimado se ha codificado con el valor `1` mientras que ser animado se codificó con el valor `0`. Nosotros podemos calcular las probabilidades con la función logístrica, la cual está disponible en R como la función `plogis`.
...
Nuestra predicción es que la probabilidad de obtener una estructura dativa preposicional es 51% si el recipiente es inanimado pero solo 23% si es animado.

La animicidad se queda un indicador significativo aun cuando la regresión logística considera otros factores, como la duración del sintagma y la pronominalidad. Jenset et al. (2018) utiliza una análisis similar con el inglés británico y descubren un factor de género: los hombres prefieren las estructuras dativas preposicionales. Sin embargo, tienen cuidado con este resultado cuando comentan que "further investigations would be necessary to assess whether any interaction with other factors is at play, for example conversation topics."

-   **Para el jueves**: Leer Brezina 6.1 y 6.5


## Los efectos mixtos

El término "mixto" ('mixed') se refiere a una regresión linear con efectos aleatorios además de los efectos "fijos" ('fijos') que vimos el martes (ej. la animicidad)

En un estudio con los corpora orales, por ejemplo, consideremos una variable como GRAMMATICAL_CATEGORY que tiene los valores sustantivo, verbo, adjetivo, etc. o la variable REGISTER que tienen los valores formal y informal. Podemos estudiar estas mismas variables en una variedad de corpus diferentes; estas variables se pueden repetir. Por otro lado, las variables como SPEAKER_ID, la cual tiene unos 692 valores en [el SpokenBNC2014](https://www.jbe-platform.com/content/journals/10.1075/ijcl.22.3.02lov), o FOLLOWING_WORD, que tienen 386 valores en los datos de los intensificadores de Brezina y no se pueden repetir. Hay muchos otros hablantes que no aparecen en el corpus; hay muchas otras palabras que pueden aparecer después de un intensificador. En un sentido, lo que tenemos en el corpus es una muestra aleatoria de los valores de las variables.

En este tipo de estudio, sería apropiado elaborar un modelo que considera el primer tipo de variable (GRAMMATICAL_CATEGORY; que se pueden repetir) como un efecto fijo; el segundo tipo (SPEAKER_ID; que no se pueden repetir) se deben considerar efectos aleatorios. Donde los efectos fijos se entienden como contrastes (ej. 0 para un SN animado, 1 para un SN inanimado), los efectos aleatorios se consideran variables aleatorias con una media de 0 y una desviación estándar que se estima de los datos (ve la página 10 de Brezina sobre los términos 'media'/'means' y 'desviación estándar').

### El ejemplo de los intensificadores

Promero, utilicemos el paquete `readxl` para poder abrir el archivo que contiene algunas observaciones anotadas del corpus. La Figura 6.9 de Brezina (página 208) presenta una muestra de estos datos.
...
La diferencia entre los adjetivos atributivos y los predicativos [se explica aquí](https://english.stackexchange.com/questions/175562/how-to-tell-if-an-adjective-is-attributive-or-predicative-efl-context/181215#181215).

El paquete `lm4` ofrece una variante de glm que se llama glmer. Esta función permite que espequemos las variables aleatorias por la adición de un término adicional en la formula del modelo. En este ejemplo, el término adicional es `(1|Speaker), lo cual significa que queremos modelar los hablantes inviduales con una interceptación ('intercept') variable.
...
summary(ml,corr=F)
```
La función `summary` nos muestra algunos astericos cerca de los efectos fijos que son estadisticamente significativos. Como observa Brezina (página 209), los hablantes mayores prefieren *very* y los contextos sintácticos con un adjetivo predicativo. ¿Qué nos dicen los efectos aleatorios? El La función de R `coef` revela una distribución especial con los log odds para cada individuo.

```{r echo=TRUE}
head(select(coef(ml)$Speaker,"(Intercept)"))
```
Consideremos la línea del resumen del model donde se presenta el AIC. El AIC es el Akaike Informational Criterion, una medida de la cualidad del modelo (ve  la sección 4.4 de Brezina, páginas 124-125). En la siguiente Tabla, se compara este modelo con uno más simple, m0, que carece del efecto aleatorio de "Speaker." Cuanto más bajo sea el valor del AIC, mejor será el modelo. Entonces, el valor de AIC del modelo m1 indica que es el modelo preferido. Sin embargo, no es siempre el caso que un modelo con más indicadores (predictors) sea mejor. De hecho, el AIC incluya una 'multa' basado en el número de parámetros que se tienen que estimar en el modelo.
...
| modelo | AIC |
|:-----|:-----|:-----|
| m0 (sin efecto aleatorio) | 1755.5 |
| m1 (hablante como efecto aleatorio) | 1624.8 |

El uso de los modelos de efectos mixtos se ha hecho popular en diferentes campos de investigación. [El trabajo de Johnson (2014)](http://danielezrajohnson.com/johnson_2014b.pdf) describe algunas de las ventajas que tiene esta técnica para la sociolingüística en contraste con el método previo estándar, VARBRUL. VARBRUL (a través de GOLDVARB) era un forma de de regresión logística no-mixta. El trabajo de [Drager y Hay (2012)](https://www-proquest-com.proxy-remote.galib.uga.edu/docview/1008896594/A9CC2FD7DB2C485EPQ/3?accountid=14537) aplica los modelos mixtos a los grupos de chicas de colegio en Nueva Zelanda. Además, [Jaeger (2008)](https://www-sciencedirect-com.proxy-remote.galib.uga.edu/science/article/pii/S0749596X07001337) también propone que los modelos de efectos mixtso son mejores para la psicolingüística, comparado con el método estándar previo, ANOVA.

-  Para el martes, quiero que leáis el capítulo de [Kolbe-Hanna y Szmrecsanyi (2015)](https://uga.view.usg.edu/d2l/le/content/2055335/viewContent/32560333/View) sobre la variación gramatical. En este capítulo, se menciona "Somers D_{xy}". Para más información sobre esto, se puede consulta la sección 6.3.1 de [Baayen (2008)](http://www.sfs.uni-tuebingen.de/~hbaayen/publications/baayenCUPstats.pdf). 

# 12a Semana

## La regresión logística

En el trabajo de [Kolbe-Hanna and Szmrecsanyi (2015)](https://uga.view.usg.edu/d2l/le/content/2055335/viewContent/32560333/View), se presenta un estudio de la elisión de los complementantes en inglés con el uso del [FRED corpus](https://uga.view.usg.edu/d2l/le/content/2064782/viewContent/32546838/View). La variable dependiente es la presencia o la ausencia de la palabra "that" en los contextos descritos en la siguiente tabla:


|  | complementier position | |
|:-----|:-----|:-----|
| I think | that | this is good |
| I think | $\emptyset$ | this is good |

**Tabla 1**  Omisión del complementante (p. 107, Tabla 4.1)

¿Cuáles son los factores que condicionen la manifestación de un complementante explícito? Dr. Kolbe-Hanna ha compartido sus datos con el profesor Hale y se encuentran en el archivo `compl_spoken2.csv`. En este archivo, las variables dependientes se incluyen como columnas; es decir, la variable dependiente ("that" vs. omisión de "that") entre otras.

-  El pronombre de la cláusula principal (`matrix_subject_type_3`)
-  La negación (o no) de la cláusula principal (`matrix_negation`)
-  Si el primer elemento de la cláusula subordinanda es un pronombre (`complement_subject`)

Estos datos se pueden abrir así:

```{r}
compl <- read.table("compl_spoken2.csv", header=T, sep=",", dec=".", stringsAsFactors=T)
```

La función de `read.table` eximina el archivo CSV (comma-separated-value) y asegura que todas las columnas con valores de series de carácteres ("character-string-valued columns") se tratarán como factores. Luego, esta función produce un data frame en R con el nombre `compl`.

Los factores independientes se describen individualmente en la sección 2.2.1 del capítulo de Kolbe-Hanna and Szmrecsanyi. Su sistema de codificación de los datos se describe en [el Codebook](https://uga.view.usg.edu/d2l/le/content/2055335/viewContent/32591365/View).

La estrategia de Kolbe-Hanna y Szrecsanyi es empezar con un modelo que contiene todos los factores y después removerlos uno por uno par encontrar la explicación más mezquina. Aquí repetimos los pasos importantes de su análsis, empezando con la verificación de una suposición importante de la regresión logística: que cada factor predice el resultado de una manera distinta.
...
El argumento adicional `control=glmerControl` mande a R que intente lo máximo cuando crea un modelo, un proceso que evita los fallos que tienen que ver con la falta de convergencia. ¿Se reconocen los efectos aletorios especificados en este modelo?

### La (multi)colinearidad

Según [Dylan Glynn](http://www.dsglynn.univ-paris8.fr/) en una presentación reciente, "Multicollinearity is typically the biggest problem in corpus linguistics." Los indicadores son colineares si se pueden re-escribir como la suma ponderada de uno o más de los otros indicadores que también existen como parte del modelo. Tener dos variables que son similares de esta manera aumenta la posibilidad de que rechazemos la hipótesis nula cuando sea verdad.

La idea de la colinearidad parece extraña cuando se compara con los indicadores categóricos, pero todavía podemos verificar con un cálculo de los factores de inflación variante ("variance inflation predictors"). El VIF es una proporción de la varianza de un coeficiente particular $\beta$_j cuando el resultado del modelo completo (es decir, con todos los factores) la comparación con la varianza de $\beta$_j de un modelo que solo tuviera este como el único indicador. En la literatura, hay cierto debata sobre el nivel de utilidad del VIF. Según Glynn, se debe indicar cualquier número que excede 4.
...
Debido a que algunos de los factores tienen más de dos valores (ej. la morfología verbal tienen 4), los VIFs se identifican como "generalized" con grados de libertad que más de 1. En este caso, todos los GVIF son buenos.

### ¿Vale un indicador particular?

Para determinar si vale la pena incluir un indicador en el modelo, debemos tener algunas medidas de "goodness of fit". En la última sesión de clase, hablamos de AIC como una medida de la calidad de un modelo. Esta medida nos puede servir. Brezina describe esta medida con una ecuación (página 125) como la variación en los datos que queda sin explicación en un modelo particular. Esta variación se conoce como el "likelihood" $L$ de un "fitted model", dado los datos overvados. Podemos utilizar el "likelihood" como una prueba estadística mediante el cálculo de la probabilidad de dos "likelihoods"; es decir, sobre un modelo que incluye el indicador bajo consideración en comparación con un modelo que no lo incluye.

-  likelihood ratio $$= -2ln(\frac{L(m_o)}{L(m_1)}) = 2(logLik(m_1) - logLik(m_o)$$

Con los logoritmos, la división se convierte en la resta. Según el teorema de Wilk, este likelihood ratio tienen una distribución $\chi^{2}$ con un número de grados de libertad que equivale al número de parámetros que se remueven del modelo. Intentemos quitar el factor de morfología verbal del modelo. 

#### Remover un efecto fijo

Consideremos la hipótesis nula que la morfología verbal no tiene valor en nuestro modelo. Lo podemos quitar del modelo con [la función en R `update`](https://cran.r-project.org/doc/manuals/r-release/R-intro.html#Updating-fitted-models). El segundo argumento en esta función es un formulario en el cual el punto significa "el mismo que antes" y el signo menos significa "remover".
...
Notáos que el modelo con más indicadores siempre tendrá un mejor log-likelihood. ¿Pero es significamente mejor?

```{r echo=TRUE}
pchisq(as.numeric(noverbmorphdeviance), df=3, lower.tail=F)  # 4 - 1 = 3 degrees of freedom
```

La probabilidad de rechazar equivocadamente la hipótesis nula cuando es verdad, considerando una probabilidad de "likelihood" tan grande, es mínima, menos de 0.0000127324. Concluimos que el indicador de la morfología verbal se debe retener en el modelo. Podemos realizar esta misma prueba con la función `anova` en R.

```{r echo=TRUE}
anova(no_verb, maximal_model, test="Chisq")
```

Notemos que el mismo p-value está indicado en la fila de los resultados del `anova` que corresponde con el "maximal model". El juicio que este model sea mejor que el modelo sin el indicador de la morfología verbal es estadísticamente significativo.

#### Remover un efecto aleatorio

También podemos realizar esta misma prueba de "likelihood ration" con los efectos aleatorios, como, por ejemplo, "County" de los hablantes incluídos en el corpus de FRED.
...
Otra vez, la prueba nos indica que debemos retener este indicador.

#### La selección al revés

El proceso de quitar iterativamente los indicadores que no se quieren se conoce como la selección al revés ("backwards selection", Brezina p. 123). Podemos calcular las probabilidades de likelihood para todos los modelos posibles que se pueden obtener mediante la función `drop1` que permite la extracción de un único indicador. ¡Cuidado! Este proceso requiere cierto poder computacional.
...
Los resultados de las pruebas de las probabilidades likelihood sugieren que debamos remover algunos de los indicadores como la presencia de la negación en la cláusula principal y la probilidad de tipo-caso dentro de cincuenta palabras. Este proceso produce el siguiente modelo:
...
Si utilizamos la función `summary` con el `optimal_model`, veremos una tabla que nos muestra cuáles son los indicadores que son estadísticamente significativos según una prueba de Wald (Brezina, página 128).

El resumen en este caso corresponde a lo que se publicó en la Tabla 9 de Kolbe-Hanna & Szmrecsanyi (2015). Podemos recuperar los "odds ratios" con la producción de los exponentes de los estimados (es decir, los log-odds). Con R, este proceso se puede realizar para todos los efectos fijos a la vez: `exp(fixef(optimal_model)) (ver la página 170 de Kolbe-Hanna y Szmrecsanyi).

Consideremos ahora que pasa si el sujeto es "it" como en "it appears...". Con los valores típicos de duración de cláusula y falta de fluidez:
...
Vemos que si el sujeto es "it", la probabilidad de tener un "that" explícito aumenta de 0.21 a 0.52.
...
Este cálculo es análogo a lo que hicimos con los datos de Bresnan et al. sobre la alternancia dativa.
...
- [Portal de Corpus Históricos Iberorrománicos](http://www.corhiber.org/)
...
- [Old Spanish Textual Archive](http://oldspanishtextualarchive.org/osta/osta.php)
- [HSMS ÷ DIGITAL LIBRARY OF OLD SPANISH TEXTS](http://www.hispanicseminary.org/t&c/col/index.htm)
- [Oralia Diacrónica del Español (ODE/CORDEREGRA)](http://corpora.ugr.es/ode/index.php?action=home)
...
- [Proyecto COLA](https://blogg.hiof.no/colam-esp/): El corpus COLA posibilita el análisis del habla oral adolescente (13 a 19 años) de Madrid y de otras capitales latinoamericanas.
